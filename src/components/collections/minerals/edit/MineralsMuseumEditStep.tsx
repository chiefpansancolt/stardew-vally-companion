...
import type { MineralsMuseumEditStepProps } from "@/types";
import type { MuseumItem } from "@/types/app/game";
import { assetPath } from "@/lib/utils/assetPath";

const allMinerals = minerals().mineralItems().get() as MineralItem[];

export function MineralsMuseumEditStep({
	minerals: initial,
	onChange,
}: MineralsMuseumEditStepProps) {
	...
	const filtered = allMinerals.filter(
		(m) => !search || m.name.toLowerCase().includes(search.toLowerCase())
	);

	const foundCount = allMinerals.filter((m) => local[m.id]?.found).length;
	const donatedCount = allMinerals.filter((m) => local[m.id]?.donated).length;
		...
		<div className="space-y-4">
			<div className="flex flex-wrap items-center justify-between gap-2">
				<div className="flex gap-3">
					<span className="rounded-full bg-gray-100 px-3 py-0.5 text-[0.7rem] font-semibold text-gray-600">
						{foundCount} / {allMinerals.length} found
					</span>
					<span className="rounded-full bg-gray-100 px-3 py-0.5 text-[0.7rem] font-semibold text-gray-600">
						{donatedCount} / {allMinerals.length} donated
					</span>
				</div>
				<input
					type="text"
			...
			<div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
				{filtered.map((mineral) => {
					const found = Boolean(local[mineral.id]?.found);
					const donated = Boolean(local[mineral.id]?.donated);
					return (
						<div
							key={mineral.id}
						...
						>
							...
							<div className="flex shrink-0 gap-1">
								...
								<button
									type="button"
									onClick={() =>
										setStatus(mineral.id, donated ? found : true, !donated)
									}
									className={`cursor-pointer rounded border px-1.5 py-0.5 text-[0.6rem] font-semibold transition-colors ${
										donated
